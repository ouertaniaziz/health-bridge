...
type Props = {
  onSubmit: (pharmacist: IUser) => void;
  onCancel: () => void;
};

const pharmacistScheme = Yup.object({
...
const PharmacistForm = ({ onSubmit, onCancel }: Props) => {
  const [roles] = useFetch<{ value: string }[]>('./data/pharmacist.json', []);
  const {
    setFieldTouched,
    handleChange,
    handleBlur,
